...
class DiscreteTimeShift(Perturbation):
    ...
    The shift amount is uniformly sampled between lower_limit and upper_limit.
    ...
    def __init__(self, shift_chance: float, lower_limit: int, upper_limit: int, seed: Optional[int] = None, transformation: Optional[Dict[str, Any]] = None, track_input_profiles: bool = False):
        super().__init__(seed=seed, transformation=transformation, track_input_profiles=track_input_profiles)
        ...
        if not isinstance(self.lower_limit, int) or not isinstance(self.upper_limit, int):
            raise TypeError(f"lower_limit and upper_limit must be integers, {lower_limit} and {upper_limit} provided")

        if self.lower_limit > self.upper_limit:
            raise ValueError(f"lower_limit ({lower_limit}) must be less than or equal to upper_limit ({upper_limit})")
    ...
    def _infer_transformation(self, profiles: np.ndarray) -> Dict[str, Any]:
        ...
        shift_amounts = np.random.randint(self.lower_limit, self.upper_limit + 1, size=n_cols)

        # mask shift_amounts with do_shift
        shift_amounts[~do_shift] = 0

        shift_indices = np.random.randint(0, t_steps, size=n_cols)
    ...
    def _apply_perturbation(self, profiles: np.ndarray) -> np.ndarray:
        ...
        shift_indices = self._transformation["shift_indices"]
        ...
        for col in range(n_cols):
            ...
            if do_shift[col]:
                ...
                # shift index; sample a random timestep
                shift_index = shift_indices[col]

                if shift_amount > 0:
                    # find all indices that we want to shift forward
                    origin_lower_index = shift_index
                    origin_upper_index = t_steps - shift_amount

                    target_lower_index = origin_lower_index + shift_amount
                    target_upper_index = t_steps
                    
                    if lower_index > upper_index:
                        # no indices to shift forward, skip this column
                        continue

                    if targer_lower_index >= t_steps:
                        # no indices to shift forward, skip this column
                        continue
                    
                    # shift the values  -> E.g. [0, 1, 2, 3, 4] with shift_amount=2 and shift_index=1 -> [0, 1, 2, 1, 2] (shifted values are copied to the right)
                    # Note that the example still has a 
                    new_profiles[target_lower_index:target_upper_index, col] = new_profiles[origin_lower_index:origin_upper_index, col]

                    # Handle the problem of shifting the values to the right -> [0, 1, 2, 1, 2] -> [0, 1, 1, 1, 2]
                    new_profiles[origin_lower_index:target_lower_index + 1, col] = new_profiles[origin_lower_index, col]



                elif shift_amount < 0:
                    origin_lower_index = shift_index - shift_amount  # minus since negative
                    origin_upper_index = t_steps

                    target_lower_index = shift_index
                    target_upper_index = shift_index - shift_amount

                    if origin_lower_index < 0:
                        # no indices to shift backward, skip this column
                        continue

                    if target_upper_index < 0:
                        # no indices to shift backward, skip this column
                        continue

                    # shift the values  -> E.g. [0, 1, 2, 3, 4] with shift_amount=-2 and shift_index=2 -> [0, 1, 4, ?, ?] (shifted values are copied to the left)
                    new_profiles[target_lower_index:target_upper_index, col] = new_profiles[origin_lower_index:origin_upper_index, col]

                    # Handle the problem of shifting the values to the left -> [0, 1, 4, ?, ?] -> [0, 1, 4, 4, 4]
                    new_profiles[target_upper_index:, col] = new_profiles[origin_upper_index - 1, col]
